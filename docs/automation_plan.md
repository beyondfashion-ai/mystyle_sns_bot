...
### C. AI Content Generation (Hybrid LLM)
...
### D. Editorial Evolution (에디토리얼 자동 진화)
*   **방식:** 4단계 시간 스케일로 에디토리얼 방향을 자동 조정하는 시스템.
*   **구현:** `src/editorialEvolution.js` — Gemini 2.5 Flash로 내부 분석 후 Firestore에 지시문 저장.
*   **스케줄:**
    *   **일간 (01:00 KST):** 어제 반응 기반 톤/초점 미세 조정.
...
2.  **공통 봇 고지 (Bot Disclosure):**
    *   모든 자동 게시물 하단에 `(Generated by KALEI AI)` 또는 `[AI 스타일링 영감]` 문구를 자동으로 삽입합니다.
3.  **Instagram 이미지 URL 검증:**
    *   Instagram API는 공개된 URL만 허용하므로, 게시 전 해당 URL이 외부에서 접근 가능한지 체크하는 로직을 추가합니다.
4.  **로컬 보안 관리 (.env.local):**
    *   모든 API 키와 시크릿은 `.env.local` 파일에서만 관리하며, 깃허브 등 외부 저장소에 절대 노출되지 않도록 `.gitignore`에 등록하여 관리합니다.
