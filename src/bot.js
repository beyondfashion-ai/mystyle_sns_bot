...
import { IMAGE_DOWNLOAD_TIMEOUT_MS, URL_CHECK_TIMEOUT_MS, X_HOURLY_LIMIT as _X_HOURLY_LIMIT, X_DAILY_LIMIT as _X_DAILY_LIMIT, ONE_HOUR_MS, ONE_DAY_MS, COLLECTIONS } from "./config.js";
...
export { _X_HOURLY_LIMIT as X_HOURLY_LIMIT, _X_DAILY_LIMIT as X_DAILY_LIMIT };
...
function checkXRateLimit(userId = "default") {
    ...
    if (now >= entry.hourlyResetAt) {
        entry.hourlyCount = 0;
        entry.hourlyResetAt = now + ONE_HOUR_MS;
    }
    if (now >= entry.dailyResetAt) {
        entry.dailyCount = 0;
        entry.dailyResetAt = now + ONE_DAY_MS;
    }

    if (entry.hourlyCount >= _X_HOURLY_LIMIT) {
        return { allowed: false, reason: `Hourly limit reached (${_X_HOURLY_LIMIT}/hour)` };
    }
    if (entry.dailyCount >= _X_DAILY_LIMIT) {
        return { allowed: false, reason: `Daily limit reached (${_X_DAILY_LIMIT}/day)` };
    }
...
async function savePostRecord({ text, platforms, imageUrls, results }) {
    if (!db) return;
    try {
        await db.collection(COLLECTIONS.PENDING_POSTS).add({
            text,
            platforms,
            imageUrls: imageUrls || [],
            results,
...
export function getRateLimitStatus(userId = "default") {
    ...
    if (!entry) {
        return {
            hourlyCount: 0,
            dailyCount: 0,
            hourlyLimit: _X_HOURLY_LIMIT,
            dailyLimit: _X_DAILY_LIMIT,
            hourlyResetIn: '60분',
            dailyResetIn: '24시간',
        };
    }
